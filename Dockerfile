...
RUN addgroup -S -g 10015 prtg_offline_sensor_reporter && \
    adduser -S -u 10014 -G prtg_offline_sensor_reporter prtg_offline_sensor_reporter
USER prtg_offline_sensor_reporter:prtg_offline_sensor_reporter

# Set the entry for the container to run the app.
ENTRYPOINT ["python", "prtg_offline_sensor_reporter.py"]
